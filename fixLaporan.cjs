...
const sIdx = content.indexOf("    const getLaporanData = (tabId: 'laporan'");
const eIdx = content.indexOf("\n  const dataAlaman =", sIdx);
if (sIdx !== -1 && eIdx !== -1) {
...
`  const getLaporanData = (tabId: 'laporan' | 'pelean' | 'alaman' | 'perorangan' | 'distribusi') => {
    ...
    if (tabId === 'alaman') {
      const peroranganData = mappedData.filter(m => false); // we can't recursively call it this way, wait 
      // actually, just use getLaporanData('perorangan') recursively, but wait, the previous code had peroranganData as a separate map
    }
